...
public class Pokemon {
    ...
    public static Pokemon fromJson(JsonObject jsonObject, int gen) {
        JsonObject arraySpecieName = jsonObject.getAsJsonObject("species");
        String name = arraySpecieName.get("name").getAsString();
        if(gen == 0){
            gen = getGeneration(jsonObject);
        }
    ...
    public static int getGenerationByID(JsonObject jsonObject) {
    ...
    public static int getGeneration(JsonObject jsonObject) {
        JsonArray gameIndicesArray = jsonObject.getAsJsonArray("game_indices");
        JsonObject firstGameIndex = gameIndicesArray.get(0).getAsJsonObject();
        JsonObject versionObject = firstGameIndex.getAsJsonObject("version");
